...
3. 語音準確度（integrity）：根據轉寫文字中的錯別字、用詞不當或不合語境來推斷可能的語音錯誤。若文字內容完全無法理解或嚴重離題，則給予低分。
...
- 完整性（completeness）：回答是否完整回應子問題。必須包含問題中的關鍵名詞或動詞，不能只給孤立的詞彙或片段。例如，若問題是「媽媽穿著什麼？」，回答「裙子」為不完整，應為「媽媽穿著裙子」；若能補充修飾語（如「一條漂亮的裙子」）則更佳。
- 語音準確度（integrity）：根據轉寫文字中的錯別字、用詞不當或不合語境來推斷可能的語音錯誤。若文字內容完全無法理解或嚴重離題，則給予低分。
