...
3. 語音準確度（integrity）：根據轉寫文字的流暢度和與問題的相關性來推斷發音清晰度。若文字流暢且相關，給高分；若文字難懂或離題，給低分。

最後給出總分（0-10，精確到小數點後一位）和具體建議。

要求：
- 所有建議和評語必須使用小學生能聽懂的簡單詞語，如「用詞多不多」、「故事說得清不清楚」。
- 指出問題時要用鼓勵和幫助的語氣，不要責備或嚴厲批評，重點是告訴學生怎樣可以說得更好。
- 絕對不要在評語中提及「寫」、「寫字」、「寫對」、「寫錯」或具體的錯別字，因為輸入是語音轉寫，不是書面文字。
...
- 語音準確度（integrity）：根據轉寫文字的流暢度和與問題的相關性來推斷發音清晰度。若文字流暢且相關，給高分；若文字難懂或離題，給低分。

然後計算總分（0-10，可為各子問題的平均分，精確到小數點後一位）並給出整體建議。

要求：
- 所有評語和建議必須使用小學生能聽懂的簡單詞語，如「回答得完不完全」、「有沒有說清楚」。
- 指出問題時要用鼓勵和幫助的語氣，不要責備或嚴厲批評，重點是告訴學生怎樣可以說得更好。
- 絕對不要在評語中提及「寫」、「寫字」、「寫對」、「寫錯」或具體的錯別字，因為輸入是語音轉寫，不是書面文字。
- 所有文字必須是繁體中文標準書面語，不得使用任何粵語口語（如「嘅」、「咁」、「佢」等）。
- 輸出 JSON 格式：
